...
io.on('connection', (socket) => {
    ...
    socket.on('requestPlayerInfo', (data) => {
        ...
        if (targetPlayer) {
            ...
            socket.emit('playerInfo', { x: targetPlayer.basePos.x, y: targetPlayer.basePos.y, lockedUntil });
        }
    });
    ...
    // Attempt recovery (victim tries to take a cat back from thief if close enough)
    socket.on('attemptRecover', (data) => {
        const thiefName = data && data.thiefUsername;
        const victim = players[socket.id];
        if (!victim || !thiefName) return;
        // Find thief player
        let thiefPlayer = null;
        for (const [id, p] of Object.entries(players)) {
            if (p.username === thiefName) { thiefPlayer = p; break; }
        }
        if (!thiefPlayer) { socket.emit('recoverFailed', 'Thief not found'); return; }
        const dist = Math.hypot(thiefPlayer.x - victim.x, thiefPlayer.y - victim.y);
        const RECOVER_DIST = 150; // pixels
        if (dist > RECOVER_DIST) { socket.emit('recoverFailed', 'Too far to recover'); return; }

        const thiefBase = bases[thiefName] || [];
        if (!thiefBase.length) { socket.emit('recoverFailed', 'No cats to recover'); return; }

        // Pop one cat from thief and give to victim
        const recovered = thiefBase.pop();
        if (!bases[victim.username]) bases[victim.username] = [];
        bases[victim.username].push(recovered);

        io.emit('recoverResult', { thiefName, victimName: victim.username, cat: recovered, bases });
    });
