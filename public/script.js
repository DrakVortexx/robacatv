...
// When server instructs to teleport, move our player to that position and inform server
socket.on('teleportTo', (data) => {
...
function renderWorld() {
    ...
    gameCanvas.onclick = (ev) => {
        ...
        for (const [id, player] of Object.entries(worldState.players)) {
            ...
            if (dist < 30) {
                // Teleport to player's base: emit event to server asking for their base pos
                if (socket) {
                    socket.emit('requestTeleport', { targetUsername: player.username });
                }
                break;
            }
        }
