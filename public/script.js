...
function login(username, password) {
    ...
    const savedData = localStorage.getItem(`robacat_user_${username}`);
...
function createNewAccount(username, passwordHash) {
    const newUser = {
    ...
    localStorage.setItem(`robacat_user_${username}`, JSON.stringify(newUser));
    loadGame(username, newUser);
}
...
function saveGame() {
    ...
    const savedData = localStorage.getItem(`robacat_user_${gameState.username}`);
    if (savedData) {
        const userData = JSON.parse(savedData);
        userData.gameState = gameState;
        localStorage.setItem(`robacat_user_${gameState.username}`, JSON.stringify(userData));
