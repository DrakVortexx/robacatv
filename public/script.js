...
// Generate ~100 unique cat types programmatically while preserving some handcrafted favorites.
function generateCatTypes(targetCount = 100) {
    // Handcrafted favorites (will be included first)
    const handcrafted = [
    ...
    // Kid-friendly name parts
    const adjectives = ['Sunny','Fluffy','Bouncy','Sparkle','Cuddly','Poppy','Merry','Jazzy','Zippy','Twinkle','Giggly','Peppy','Pudding','Biscuit','Nibble','Pepper','Maple','Bunny','Cookie','Muffin','Doodle','Butter','Sprinkle','Pickles','Button','Marsh','Cupcake','Waffle','Pip','Noodle','Giggle'];
    const nouns = ['Paws','Whisker','Sprout','Tail','Bean','Nose','Button','Snuggle','Cloud','Bounce','Skipper','Hop','Tumble','Puff','Spark','Dew','Pebble','Jelly','Cup','Buddy','Bloop','Sprocket','Daisy','Berry','Mango','Pumpkin','Pip','Toffee','Bubbles','Twix'];

    const emojis = ['�','�','😻','�','�','�','�','�','�','�','�‍⬛','🦁','�','�','�','🐾','🧶','🎩','🕶️','👑','🌟','✨','💎','🍣','','🍕','🎁','🎉','🚀','🌈','🔥','❄️','🌊','🌿','⚡','🌙','☀️','🍀','🎃','🍁','🍒','🍇','🧸','🪁','🛸','🎧','📦','🖼️','🪄','🧩','🔮','🛍️'];

    const rarities = [
        { name: 'common', weight: 60, incomeRange: [1,4], valueMultiplier: 1, costMultiplier: 1 },
        { name: 'uncommon', weight: 20, incomeRange: [3,7], valueMultiplier: 2, costMultiplier: 2 },
        { name: 'rare', weight: 12, incomeRange: [8,20], valueMultiplier: 5, costMultiplier: 5 },
        { name: 'legendary', weight: 6, incomeRange: [30,200], valueMultiplier: 20, costMultiplier: 20 },
        { name: 'mythical', weight: 2, incomeRange: [200,2000], valueMultiplier: 100, costMultiplier: 100 }
    ];

    const types = [];
    // Start with handcrafted favorites
    handcrafted.forEach(c => types.push(Object.assign({}, c)));
    ...
    // Compose kid-friendly unique names by combining adjective + noun + small counter
    const usedNames = new Set(types.map(t => t.name));

    while (idx < targetCount) {
        const rarityInfo = pickRarity();
        const adj = adjectives[(idx + Math.floor(Math.random() * adjectives.length)) % adjectives.length];
        const noun = nouns[(idx + Math.floor(Math.random() * nouns.length)) % nouns.length];
        let name = `${adj} ${noun}`;
        if (usedNames.has(name)) name = `${name} ${idx}`;
        usedNames.add(name);

        const emoji = emojis[idx % emojis.length];
        const baseIncome = Math.round(randBetween(rarityInfo.incomeRange[0], rarityInfo.incomeRange[1]));
        const value = Math.max(1, Math.round(baseIncome * 5 * rarityInfo.valueMultiplier));
        const cost = Math.max(1, Math.round(value * 0.8 * rarityInfo.costMultiplier));

        const cat = {
            id: `cat_${String(idx+1).padStart(3,'0')}`,
            name: `${name}`,
            rarity: rarityInfo.name,
            chance: Math.max(0.01, Math.round((rarityInfo.weight / 100) * 10) / 10),
            income: baseIncome,
            value: value,
            cost: cost,
            emoji: emoji
    ...
    // Ensure we return exactly targetCount entries
    return types.slice(0, targetCount);
}
...
socket.on('newPlayer', (player) => {
    ...
    modalSteal.onclick = () => {
        // start client-side 6s countdown then emit trySteal
        beginSteal(data.username);
        otherBaseModal.classList.add('hidden');
    };
...
// When a steal is incoming to this player, prompt a simple 1-digit addition question
socket.on('incomingSteal', (data) => {
    if (!data) return;
    const q = data.question; // e.g., '1+3'
    const id = data.id;
    const thief = data.thiefName;
    const ans = prompt(`Incoming steal by ${thief}! Answer ${q} to keep your cat:`);
    // send answer back to server
    if (ans !== null) {
        socket.emit('answerSteal', { id, answer: Number(ans) });
    }
});
    ...
    if (self) {
        ...
        if (nearbyBase) {
            ...
            if (keys.e && socket) {
                ...
                beginSteal(nearbyBase.username);
            }
        } else {
...
// Start a steal countdown (6s) then emit trySteal
let _stealInProgress = false;
function beginSteal(targetUsername) {
    if (!socket) { alert('Not connected'); return; }
    if (_stealInProgress) { alert('Already stealing...'); return; }
    _stealInProgress = true;
    let sec = 6;
    const originalText = interactionMsg.textContent;
    interactionMsg.textContent = `Stealing ${targetUsername} in ${sec}s...`;
    interactionMsg.classList.remove('hidden');
    const iv = setInterval(() => {
        ...
        if (sec > 0) {
            ...
            clearInterval(iv);
            interactionMsg.classList.add('hidden');
            socket.emit('trySteal', targetUsername);
            _stealInProgress = false;
        }
    }, 1000);
}
